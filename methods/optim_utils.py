# Source: https://github.com/YuxinWenRick/tree-ring-watermark/blob/main/optim_utils.py#L126
...
def get_watermarking_mask(init_latents_w, device):
    watermarking_mask = torch.zeros(init_latents_w.shape, dtype=torch.bool).to(device)

    # if args.w_mask_shape == 'circle':
    np_mask = circle_mask(init_latents_w.shape[-1], r=10) #r=args.w_radius)
...
def get_watermarking_pattern(pipe, device, key, shape, dtype):
    w_pattern = "const"
    set_random_seed(10799)
    w_pattern_const = (key - 4) / 8
    gt_init = torch.randn(*shape, device=device, dtype = dtype)
...
def eval_watermark(reversed_latents_no_w, reversed_latents_w, watermarking_mask, gt_patch):
    w_measurement = "l1-complex"
